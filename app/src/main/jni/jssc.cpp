 ...
 */
JNIEXPORT jstring JNICALL Java_com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_getNativeLibraryVersion(JNIEnv *env, jobject object) {
 ...
 */
JNIEXPORT jlong JNICALL Java_com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_openPort(JNIEnv *env, jobject object, jstring portName, jboolean useTIOCEXCL){
    ...
    if(hComm != -1){
        ...
        if(tcgetattr(hComm, settings) == 0){
            ...
            hComm = com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_ERR_INCORRECT_SERIAL_PORT;//-4;
        }
        delete settings;
        //<- since 2.2.0
    }
        ...
        if(errno == EBUSY){//Port busy
            hComm = com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_ERR_PORT_BUSY;//-1
        }
        else if(errno == ENOENT){//Port not found
            hComm = com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_ERR_PORT_NOT_FOUND;//-2;
        }//-> since 2.2.0
        else if(errno == EACCES){//Permission denied
            hComm = com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_ERR_PERMISSION_DENIED;//-3;
        }
        else {
            hComm = com_example_gourdboy_serialportdemo_jssc_SerialNativeInterface_ERR_PORT_NOT_FOUND;//-2;
        }//<- since 2.2.0
